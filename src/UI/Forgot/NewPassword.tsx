...
export const NewPassword = React.memo(() => {
    ...
    const isSend = useSelector<IAppStore, boolean>(state => state.newPasswordReducer.isSend)
    ...
    if (isSend) return <CheckEmail/>  // КТО ДЕЛАЕТ Логин, тут логин компоненту вставить
            ...
            <div className={styles.password}>
                <div className={styles.password__body}>
                    ...
                    <div className={styles.password__inp__wrap}>
                        <input placeholder={"Password"} type={icon ? 'password' : 'text'}
                               value={valueInput}
                               onChange={onInputChange}
                               className={styles.password__input}/>
                        <img className={styles.password__icon} src={icon ? eye : eyeSlash}
                             alt="" onClick={onIconHandler}/>
                    </div>
                    <div className={styles.password__text}>Create new password and we will
